...
- 🖥 **Admin 控制台** — 浅色专业界面：API Key 工具条、剩余积分/请求/Token 用量统计卡、账号状态、签到记录、请求日志（时间带日期）
...
浏览器打开 `http://127.0.0.1:8091/admin`（端口以 config.json 的 `listen` 为准）：

| 模块 | 说明 |
|---|---|
| API Key 工具条 | 主 Key 查看/复制/修改 |
| 密钥管理 | 创建/启停/删除附加密钥，掩码显示、一键复制，即时生效 |
| 用量统计 | 剩余积分、请求数、输入/输出 tokens、实扣积分、平均首字 |
| 账号 | 每账号健康状态、积分、成功/失败/在途计数 |
| 签到 | 上次签到结果 + 下次自动签到时间，支持手动触发 |
| 模型白名单 | 勾选放行的模型（不勾 = 放行全部），即时生效 |
| 请求日志 | 最近请求（时间带日期/模型/状态/首字/TTFB/tokens/积分/耗时），5s 自动刷新 |

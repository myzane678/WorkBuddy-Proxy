...
func (p *Pool) pick(tried map[string]bool) *auth.Auth {
	...
	if len(eligible) == 0 {
		...
		// 注意：cands[0] 只是权重/uid 序的第一个，未必是最旧的——并发爆发下
		// 各账号 lastUsed 同刻被置位、权重不分伯仲，直接取 cands[0] 会固定
		// 收敛到 uid 最小的账号，退化为雪崩。先求全局最旧；存在并列最旧
		// （同刻被用过）时改走加权随机打散。
		oldest := cands[0]
		...
		tiedOldest := false
		for _, c := range cands {
			if c != oldest && !c.lastUsed.After(oldest.lastUsed) {
				tiedOldest = true
				break
			}
		}

...
type chatStat struct {
	...
	inToks int // 输入 tokens，<0 表示 usage 缺失
	credit float64 // 本次实扣积分（usage.credit），<0 表示缺失
	status int
...
func newChatStat(now time.Time, body []byte, stream bool) *chatStat {
	...
	return &chatStat{start: now, model: parseModelFromBody(body), mode: mode, toks: -1, inToks: -1, credit: -1}
}

// done 幂等落一行表格日志，并记入 /admin 请求日志环形缓冲。
func (s *chatStat) done() {
	...
	logChatRow(seq, s.ttfb, total, s.model, s.mode, s.uid, s.status, s.toks)
...
type chatStatsReader struct {
	...
	tokens   int
	inToks   int     // 末帧 usage.prompt_tokens
	credit   float64 // 末帧 usage.credit，<0 = 缺失
	pend     []byte // 已读未返回的行缓存
}

// newChatStatsReaderSince 以 since 为 TTFB 计时起点（通常是请求进入 handler 的时刻）。
func newChatStatsReaderSince(r io.Reader, since time.Time) *chatStatsReader {
	return &chatStatsReader{br: bufio.NewReaderSize(r, 64*1024), start: since, credit: -1}
}
...
// InputTokens 返回末帧 usage.prompt_tokens；无 usage 时 -1。
func (s *chatStatsReader) InputTokens() int { return s.inToks }

// Credit 返回末帧 usage.credit（实扣积分）；无 usage 时 -1。
...
func (s *chatStatsReader) parseSSELine(line string) {
	...
	s.inToks = chunk.Usage.PromptTokens
...
func logChatRow(seq int64, ttfb, total time.Duration, model, mode, uid string, status int, toks int) {
	if !chatLogEnabled {
		return
	}
	if len(model) > 11 {
...
// ttfbOrMinusOne TTFB 为 0（无帧）时返回 -1，便于 JSON 里区分 "无" 与 0ms。
func ttfbOrMinusOne(ttfb time.Duration) int64 {
	if ttfb <= 0 {
		return -1
	}
	return ttfb.Milliseconds()
}

// syncInputTokens 从 Aggregate 响应提取 usage.prompt_tokens；缺失返回 -1。
func syncInputTokens(resp map[string]any) int {
	u, ok := resp["usage"].(map[string]any)
	if !ok {
		return -1
	}
...
// syncCredit 从 Aggregate 响应提取 usage.credit（本次实扣积分）；缺失返回 -1。
func syncCredit(resp map[string]any) float64 {
	u, ok := resp["usage"].(map[string]any)
	if !ok {
		return -1
	}

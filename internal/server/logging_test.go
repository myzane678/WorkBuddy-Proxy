...
func TestChatStatsReaderTokensFromUsage(t *testing.T) {
	// 起点前移 1ms：strings.Reader 读取过快时 time.Since 可能为 0，机器速度不该影响断言。
...
func TestLogChatRowFormat(t *testing.T) {
	...
	out := captureStdout(t, func() {
		logChatRow(1, 412*time.Millisecond, 27100*time.Millisecond, "deepseek-v4-flash", "stream", "00e26541abcdef", http.StatusOK, 1234)
	})
...
func TestLogChatRowSeqIncrements(t *testing.T) {
	withChatLog(t)
	out := captureStdout(t, func() {
		// seq 现由 chatStat.done() 生成后传入（chatSeq.Add(1)），logChatRow 只负责渲染。
		logChatRow(1, 0, time.Second, "m", "sync", "u", 200, 1)
		logChatRow(2, 0, time.Second, "m", "sync", "u", 200, 1)
	})

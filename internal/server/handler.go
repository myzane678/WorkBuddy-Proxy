...
type Config struct {
	...
	// Allowlist 模型白名单：空 = 全部放行；非空时 /v1/models 过滤 + chat 校验 403。
	// admin 改白名单直接替换本 slice（引用不可变，并发读安全）。
	Allowlist []string
	// APIKeys 附加密钥列表：与主 key 并存，任一启用项命中即放行。
	// admin 变更时整体原子替换（引用不可变，并发读安全）。
	APIKeys []APIKeyEntry
	// Admin 状态页依赖（可零值：对应能力降级）。
	Admin AdminExtras
}
...
func (h *Handler) withAuth(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if h.cfg.APIKey != "" {
			...
			ok := strings.HasPrefix(authz, "Bearer ") && key == h.cfg.APIKey
			if !ok { // 附加密钥列表：任一启用项命中即放行
...
// models 返回模型列表：优先动态（缓存 1h），失败回退静态表；白名单非空时过滤。
func (h *Handler) models(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, http.StatusOK, map[string]any{
		"object": "list",
		"data":   filterModels(h.modelList(), h.cfg.Allowlist),
	})
}

// filterModels 按白名单 id 过滤模型条目；白名单空 = 全部放行。
func filterModels(list []map[string]any, allowlist []string) []map[string]any {
	if len(allowlist) == 0 {
		return list
	}
	out := make([]map[string]any, 0, len(list))
	for _, m := range list {
		if id, ok := m["id"].(string); ok && allowlisted(allowlist, id) {
			out = append(out, m)
		}
	}
	return out
...
// allowlisted 精确匹配白名单。
func allowlisted(allowlist []string, model string) bool {
...
func (h *Handler) chatCompletions(w http.ResponseWriter, r *http.Request) {
	...
	// 白名单门禁：非空时请求模型必须在列内（zcode-api 同语义 403 model_not_allowed）。
	if len(h.cfg.Allowlist) > 0 && !allowlisted(h.cfg.Allowlist, st.model) {
		st.status = http.StatusForbidden
		writeOpenAIError(w, http.StatusForbidden, "model_not_allowed",
			"model "+st.model+" not in allowlist")
		return
	}

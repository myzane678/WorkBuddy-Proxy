...
func TestAPIKeyAuthWithExtraKeys(t *testing.T) {
	h := NewHandler(Config{
		Pool:     testPoolWith(&auth.Auth{UID: "u1", AccessToken: "at", ExpiresAt: 9999999999}),
		Upstream: upstream.New(),
		APIKey:   "secret",
		APIKeys: []APIKeyEntry{
			{ID: "k1", Name: "on", Key: "wb-sk-live", Enabled: true},
			{ID: "k2", Name: "off", Key: "wb-sk-dead", Enabled: false},
		},
	})
	// 启用中的附加 key 放行
	req := httptest.NewRequest("GET", "/v1/models", nil)
	req.Header.Set("Authorization", "Bearer wb-sk-live")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	if rec.Code != 200 {
		t.Errorf("extra key enabled: code=%d", rec.Code)
	}
	// 禁用的附加 key 拒绝
	req = httptest.NewRequest("GET", "/v1/models", nil)
	req.Header.Set("Authorization", "Bearer wb-sk-dead")
	rec = httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	if rec.Code != 401 {
		t.Errorf("extra key disabled: code=%d", rec.Code)
	}
	// 内存替换列表后（模拟 admin 删除），原附加 key 立即失效
	h.cfg.APIKeys = nil
	req = httptest.NewRequest("GET", "/v1/models", nil)
	req.Header.Set("Authorization", "Bearer wb-sk-live")
	rec = httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	if rec.Code != 401 {
		t.Errorf("extra key removed: code=%d", rec.Code)
	}
}
